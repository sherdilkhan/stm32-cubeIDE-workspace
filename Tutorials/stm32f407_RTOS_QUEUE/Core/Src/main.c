...
void Start_print_task(void *argument)
{
  ...
  for(;;)
  {
    if (osMessageQueueGet(data_queueHandle, &distance, NULL, osWaitForever) == osOK)
    {
...
void Start_data_task(void *argument)
{
  ...
  for(;;)
  {
    ...
    distance = 1.23; // Replace with actual sensor reading
    osMessageQueuePut(data_queueHandle, &distance, 0, osWaitForever);
    osDelay(1000); // Adjust the delay as needed
  }
