...
int main(void)
{
  ...
  while (1)
  {
	      ...
	      while (__HAL_TIM_GET_COUNTER(&htim1) < 10);  // Wait until the counter reaches the us input value
	      HAL_GPIO_WritePin(TRIG_PORT, TRIG_PIN, GPIO_PIN_RESET);
	     ...
	     // HAL_TIM_Base_Start(&htim2);
	      ...
	      // Wait for ECHO_PIN to go high (pulse received)
	      __HAL_TIM_SET_COUNTER(&htim1, 0);  // Set the counter value to 0
	      pMillis = HAL_GetTick();
	      ...
	      // Send buffer content to a debugging interface like UART
	      // HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);

	      HAL_Delay(100);  // Delay before next measurement
    /* USER CODE END WHILE */
...
void delayMicroseconds(uint32_t us) {
    ...
    uint32_t delayTicks = us * (HAL_RCC_GetPCLK1Freq() / 1000000); // Convert us to ticks

    while ((__HAL_TIM_GET_COUNTER(&htim1) - start) < delayTicks) {
